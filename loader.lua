--===========================================================
-- Lil Durk 1.0  |  Key System + ESP + Aimbot + Radar
--===========================================================
...
local function loadKeyDB()
	local ok, d = pcall(readfile, "DurkKeys.json")
	if ok and d then
...
local function saveKeyDB(t)
	pcall(function() writefile("DurkKeys.json", HttpS:JSONEncode(t)) end)
end
local function isAuthed()
	return pcall(isfile, "DurkAuth.json") and pcall(readfile, "DurkAuth.json") == "1"
end
local function setAuthed(v)
	pcall(function() writefile("DurkAuth.json", v and "1" or "0") end)
end
...
--===========================================================
-- KEY SYSTEM  (DURK prefix, checksum-validated)
--===========================================================
...
function validateKey(k)
	...
	if e.hwid and e.hwid ~= "" and e.hwid ~= lp.Name then return false end
	return true
end
...
-- WEBHOOK
--===========================================================
local WH_URL = "https://discord.com/api/webhooks/1528101788279111836/UAlRKIoJ_Js3xC_-VxOrTvH-k9PjhB8KJ1Vgtao-AI1qgBBs8AQpSjA0YHcL4YQtK6q5"
local function sendWebhook(key, expires, duration)
	local expStr = os.date("%Y-%m-%d %H:%M", expires)
	local buyerMsg = ">>> **Thank you for purchasing Durk Software!** 🎉\n```\n" .. key .. "\n```\n📅 Expires: " .. expStr .. "\n\n**Instructions:**\n1️⃣ Load the script in your executor\n2️⃣ Enter your license key\n3️⃣ Press RightShift to open the menu\n4️⃣ Configure and enjoy!"
	local data = {embeds = {{
		title = "Lil Durk Software — New License",
		color = 0xc83250,
		fields = {
		...
		footer = {text = "Lil Durk 1.0 | All sales are recorded"}
	}}}
	...
	print("Thanks for purchasing Durk Software!")
	print("Key: " .. key)
...
local function saveCfg()
	pcall(function() writefile("DurkCfg.json", HttpS:JSONEncode(cfg)) end)
end
local function loadCfg()
	pcall(function()
		if isfile("DurkCfg.json") then
			local d = readfile("DurkCfg.json")
...
-- DRAWING POOL
--===========================================================
local pool = {box = {}, name = {}, hp = {}, dist = {}, line = {}}
local Dr_OK = pcall(Drawing.new, "Line")
local function mkDr(t)
	if Dr_OK then return Drawing.new(t) end
	return setmetatable({}, {__index = function() return function() end end})
end
if not Dr_OK then warn("[LD] Drawing API unavailable — visuals disabled.") end
for _ = 1, 24 do
	local b = {}
	for _, k in ipairs({"t", "b", "l", "r"}) do b[k] = mkDr("Line"); b[k].Thickness = 1.2; b[k].ZIndex = 999 end
...
local function hideViz()
	panicked = true
	for _, t in pairs(pool) do for _, o in pairs(t) do o.Visible = false end end
	for _, o in pairs(rDt) do o.Visible = false end
	for _, o in ipairs({rBg, rBd, rCt, rPv, fovC}) do o.Visible = false end
	if mainGui then mainGui.Enabled = false end
end
local function showViz()
	panicked = false
	if mainGui then mainGui.Enabled = true end
	for _, t in pairs(pool) do for _, o in pairs(t) do o.Visible = true end end
	for _, o in pairs(rDt) do o.Visible = true end
	for _, o in ipairs({rBg, rBd, rCt, rPv, fovC}) do o.Visible = true end
end
...
local function nuke()
	...
	for _, t in pairs(pool) do for _, o in pairs(t) do pcall(o.Destroy, o) end; t = {} end
	for _, o in pairs(rDt) do pcall(o.Destroy, o) end
	for _, o in ipairs({rBg, rBd, rCt, rPv, fovC}) do pcall(o.Destroy, o) end
	...
	pcall(warn, "[Lil Durk] Nuked. F3 clean exit.")
end

--===========================================================
-- GUI  (tabbed, lava theme)
--===========================================================
local mainGui = Instance.new("ScreenGui")
mainGui.Name = "LD_Menu"; mainGui.ResetOnSpawn = false; mainGui.Enabled = false; mainGui.DisplayOrder = 999
local parent = CoreGui or lp:FindFirstChildOfClass("PlayerGui")
pcall(function() mainGui.Parent = parent end)

local C_BG = c3(14, 12, 18); local C_FG = c3(24, 22, 30); local C_AC = c3(200, 50, 80)
local C_TX = c3(215, 215, 220); local C_DM = c3(110, 110, 120)
local C_RD = c3(220, 50, 50); local C_GN = c3(50, 200, 80)
...
do
	...
	lL.BackgroundTransparency = 1; lL.Size = UDim2.new(1, 0, 1, 0); lL.ZIndex = 0; lL.Parent = frm
	local LAV = {c3(255,80,10), c3(220,40,20), c3(180,20,10), c3(255,200,50), c3(255,120,20)}
	for i = 1, 25 do
		local p = Instance.new("Frame")
		p.BackgroundColor3 = LAV[ri(1, #LAV)]; p.BorderSizePixel = 0
		local sz = rn(3, 9)
		p.Size = UDim2.new(0, sz, 0, sz * rn(2, 4))
		p.Position = UDim2.new(0, ri(-10, 430), 0, ri(-540, -10)); p.ZIndex = 0; p.Parent = lL
		local c = Instance.new("UICorner"); c.CornerRadius = UDim.new(0, 2)
		pcall(function() c.Parent = p end)
		table.insert(lavP, {o=p, sp=rn(20, 65), sw=rn(0.3, 1.5), ss=rn(1, 3), ph=rn(0, 6.28)})
	end
	local gL = Instance.new("Frame")
	gL.BackgroundTransparency = 1; gL.Size = UDim2.new(1, 0, 0, 60); gL.Position = UDim2.new(0, 0, 1, -60); gL.ZIndex = 0; gL.Parent = lL
	local gd = Instance.new("UIGradient")
	gd.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, c3(200,50,80)), ColorSequenceKeypoint.new(1, c3(14,12,18))})
	gd.Rotation = 90; pcall(function() gd.Parent = gL end)
end

-- Title bar
local tbar = Instance.new("Frame")
tbar.BackgroundColor3 = C_AC; tbar.BorderSizePixel = 0; tbar.Size = UDim2.new(1, 0, 0, 34)
tbar.Parent = frm

local ttl = Instance.new("TextLabel")
ttl.BackgroundTransparency = 1; ttl.Size = UDim2.new(1, -40, 1, 0); ttl.Position = UDim2.new(0, 12, 0, 0)
ttl.Text = "Lil Durk 1.0"; ttl.TextColor3 = C_TX; ttl.Font = Enum.Font.GothamBold; ttl.TextSize = 16
ttl.TextXAlignment = Enum.TextXAlignment.Left; ttl.Parent = tbar

local closeBtn = Instance.new("TextButton")
closeBtn.BackgroundTransparency = 1; closeBtn.Size = UDim2.new(0, 34, 1, 0); closeBtn.Position = UDim2.new(1, -34, 0, 0)
closeBtn.Text = "X"; closeBtn.TextColor3 = C_TX; closeBtn.Font = Enum.Font.GothamBold; closeBtn.TextSize = 16
...
tabBar.BackgroundColor3 = c3(20, 18, 24); tabBar.BorderSizePixel = 0
tabBar.Size = UDim2.new(1, 0, 0, 28); tabBar.Position = UDim2.new(0, 0, 0, 34); tabBar.Parent = frm
...
local function makeTab(name)
	...
	box.BackgroundColor3 = C_FG; box.BorderSizePixel = 0
	box.Size = UDim2.new(1, -12, 1, -76); box.Position = UDim2.new(0, 6, 0, 62)
	box.CanvasSize = UDim2.new(0, 0, 0, 800); box.ScrollBarThickness = 4; box.Parent = frm
	box.Visible = false
	local L = {}
	L.Y = 4
	function L.lbl(t)
	...
	function L.tog(t, get, set)
		...
		b.BackgroundColor3 = C_FG; b.BorderSizePixel = 0; b.Size = UDim2.new(1, -10, 0, 28); b.Position = UDim2.new(0, 5, 0, L.Y)
		b.Text = ""; b.Parent = box; L.Y = L.Y + 32
		pcall(function() Instance.new("UICorner", b).CornerRadius = UDim.new(0, 6) end)
		local dot = Instance.new("Frame")
		dot.BackgroundColor3 = get() and C_GN or C_RD; dot.Size = UDim2.new(0, 12, 0, 12); dot.Position = UDim2.new(0, 8, 0.5, -6); dot.Parent = b
		pcall(function() Instance.new("UICorner", dot).CornerRadius = UDim.new(0, 6) end)
		local lb = Instance.new("TextLabel")
		lb.BackgroundTransparency = 1; lb.Size = UDim2.new(1, -28, 1, 0); lb.Position = UDim2.new(0, 26, 0, 0)
		lb.Text = t; lb.TextColor3 = C_TX; lb.Font = Enum.Font.Gotham; lb.TextSize = 13; lb.TextXAlignment = Enum.TextXAlignment.Left; lb.Parent = b
		b.MouseButton1Click:Connect(function()
			set(not get()); dot.BackgroundColor3 = get() and C_GN or C_RD; saveCfg()
		end); return b
	end
	function L.sldr(t, get, set, mn, mx)
		local b = Instance.new("Frame")
		b.BackgroundColor3 = C_FG; b.BorderSizePixel = 0; b.Size = UDim2.new(1, -10, 0, 34); b.Position = UDim2.new(0, 5, 0, L.Y)
		b.Parent = box; L.Y = L.Y + 38
		pcall(function() Instance.new("UICorner", b).CornerRadius = UDim.new(0, 6) end)
		local lb = Instance.new("TextLabel")
		lb.BackgroundTransparency = 1; lb.Size = UDim2.new(1, -14, 0, 16); lb.Position = UDim2.new(0, 7, 0, 0)
		...
		tr.BackgroundColor3 = c3(50, 50, 55); tr.Size = UDim2.new(1, -20, 0, 4); tr.Position = UDim2.new(0, 10, 0, 22); tr.Parent = b
		pcall(function() Instance.new("UICorner", tr).CornerRadius = UDim.new(0, 2) end)
		local fl = Instance.new("Frame")
		fl.BackgroundColor3 = C_AC; fl.BorderSizePixel = 0; fl.Size = UDim2.new((get() - mn) / (mx - mn), 0, 1, 0); fl.Parent = tr
		pcall(function() Instance.new("UICorner", fl).CornerRadius = UDim.new(0, 2) end)
		local th = Instance.new("TextButton")
		th.BackgroundColor3 = c3(220, 220, 220); th.Size = UDim2.new(0, 12, 0, 12)
		th.Position = UDim2.new((get() - mn) / (mx - mn), -6, 0.5, -6); th.Text = ""; th.BorderSizePixel = 0; th.Parent = b
		pcall(function() Instance.new("UICorner", th).CornerRadius = UDim.new(0, 6) end)
		local drag = false
		...
		UIS.InputChanged:Connect(function(i)
			if drag and i.UserInputType == Enum.UserInputType.MouseMovement then
				...
				local pct = clamp(mp.X / b.AbsoluteSize.X, 0, 1)
				local v = mn + pct * (mx - mn)
				if mx - mn > 10 then v = rnd(v) end
				set(v); lb.Text = t .. ": " .. rnd(v, 1)
				fl.Size = UDim2.new(pct, 0, 1, 0); th.Position = UDim2.new(pct, -6, 0.5, -6)
				saveCfg()
			end
		end); return b
	end
	function L.drop(t, get, set, opts)
		...
		pcall(function() Instance.new("UICorner", b).CornerRadius = UDim.new(0, 6) end)
		local lb = Instance.new("TextLabel")
		lb.BackgroundTransparency = 1; lb.Size = UDim2.new(0, 90, 1, 0); lb.Position = UDim2.new(0, 8, 0, 0)
		lb.Text = t; lb.TextColor3 = C_TX; lb.Font = Enum.Font.Gotham; lb.TextSize = 13
		lb.TextXAlignment = Enum.TextXAlignment.Left; lb.Parent = b
		local dbtn = Instance.new("TextButton")
		dbtn.BackgroundColor3 = c3(45, 45, 50); dbtn.BorderSizePixel = 0
		...
		dbtn.MouseButton1Click:Connect(function()
			...
			if open then
				...
				list = Instance.new("Frame"); list.BackgroundColor3 = c3(40, 38, 44); list.BorderSizePixel = 0
				...
				for i, opt in ipairs(opts) do
					local o = Instance.new("TextButton"); o.BackgroundColor3 = c3(40, 38, 44); o.BorderSizePixel = 0
	...
	function L.btn(t, cb)
		...
		b.Text = t; b.TextColor3 = C_TX; b.Font = Enum.Font.GothamBold; b.TextSize = 13; b.Parent = box
		pcall(function() Instance.new("UICorner", b).CornerRadius = UDim.new(0, 6) end)
...
for i, name in ipairs(TAB_NAMES) do
	...
	elseif name == "Visuals" then
		...
		L.tog("Stream Mode", function() return cfg.stream end, function(v)
			cfg.stream = v
			print("[LD] Stream mode " .. (v and "ON" .. " — everything still works, only bypasses OBS capture." or "OFF"))
		end)
	...
	elseif name == "Settings" then
		...
		L.btn("Load Config", function() loadCfg(); print("[LD] Config loaded.") end)
		L.btn("Generate Key", function()
			local k = _LD.GenKey(); if k then print("[LD] Key: " .. k) end
		end)
	end
...
local function switchTab(idx)
	...
	for j, b in ipairs(tabBtns) do
		b.BackgroundColor3 = (j == idx) and C_AC or c3(20, 18, 24)
		b.TextColor3 = (j == idx) and C_TX or C_DM
	end
...
for i, name in ipairs(TAB_NAMES) do
	...
	btn.BackgroundColor3 = (i == 1) and C_AC or c3(20, 18, 24)
	btn.BorderSizePixel = 0; btn.Size = UDim2.new(0, TAB_W, 1, 0)
	btn.Position = UDim2.new(0, (i - 1) * TAB_W, 0, 0)
	btn.Text = name; btn.TextColor3 = (i == 1) and C_TX or C_DM
...
-- KEY ENTRY GUI
--===========================================================
local keyGui = Instance.new("ScreenGui"); keyGui.Name = "LD_Key"; keyGui.ResetOnSpawn = false; keyGui.DisplayOrder = 1000; pcall(function() keyGui.Parent = parent end)
local kf = Instance.new("Frame")
kf.BackgroundColor3 = C_BG; kf.BorderSizePixel = 0; kf.Size = UDim2.new(0, 340, 0, 200); kf.Position = UDim2.new(0.5, -170, 0.5, -100)
kf.Active = true; kf.Draggable = true; kf.Parent = keyGui
local kh = Instance.new("TextLabel")
kh.BackgroundColor3 = C_AC; kh.BorderSizePixel = 0; kh.Size = UDim2.new(1, 0, 0, 34)
kh.Text = "Lil Durk 1.0  |  Key Required"; kh.TextColor3 = C_TX; kh.Font = Enum.Font.GothamBold; kh.TextSize = 15; kh.Parent = kf
local ki = Instance.new("TextBox")
ki.BackgroundColor3 = c3(24, 22, 28); ki.BorderSizePixel = 0; ki.Size = UDim2.new(1, -40, 0, 32); ki.Position = UDim2.new(0, 20, 0, 48)
...
ks.BackgroundTransparency = 1; ks.Size = UDim2.new(1, -40, 0, 16); ks.Position = UDim2.new(0, 20, 0, 84)
...
ka.BackgroundColor3 = C_AC; ka.BorderSizePixel = 0; ka.Size = UDim2.new(0, 130, 0, 32); ka.Position = UDim2.new(0.5, -65, 0, 112)
ka.Text = "AUTHENTICATE"; ka.TextColor3 = C_TX; ka.Font = Enum.Font.GothamBold; ka.TextSize = 13; ka.Parent = kf
local kx = Instance.new("TextLabel")
kx.BackgroundTransparency = 1; kx.Size = UDim2.new(1, -20, 0, 16); kx.Position = UDim2.new(0, 10, 0, 158)
kx.Text = "Enter a valid key to unlock."; kx.TextColor3 = C_DM; kx.Font = Enum.Font.Gotham; kx.TextSize = 10
...
local function doAuth()
	...
	if not ok then
		...
		warn("[LD] Auth error: " .. tostring(err))
...
local function doESP()
	...
	for _, p in ipairs(plrs) do
		repeat
			...
			if dist > 1200 then idx = idx - 1; break end
			local sz = v3(2.8, 4.6, 1.8); local cf = hrp.CFrame
			local hw, hh, hd = sz.X / 2, sz.Y / 2, sz.Z / 2
			local en = p.Team ~= lp.Team or not p.Team; local col = en and C_RD or C_GN
			...
			l.From = v2(vs.X / 2, vs.Y); l.To = v2(mnX + (mxX - mnX) / 2, mxY); l.Color = col; l.Visible = true
		until true
	end
end
...
local function doRadar()
	...
	rBg.Size = v2(rs, rs); rBg.Position = v2(rx - rs / 2, ry - rs / 2); rBg.Color = c3(0, 0, 0); rBg.Visible = true
...
_LD.GenKey = function(duration)
	...
	local db = loadKeyDB()
	if db[k] then
		print("[LD] Duplicate key, regenerating...")
...
_LD.FreezeKey = function(key)
	local db = loadKeyDB()
	if db[key] then db[key].frozen = true; saveKeyDB(db); print("[LD] Frozen: " .. key); return true end
	return false
end
_LD.UnfreezeKey = function(key)
	local db = loadKeyDB()
	if db[key] then db[key].frozen = false; saveKeyDB(db); print("[LD] Unfrozen: " .. key); return true end
	return false
end
_LD.SetHWID = function(key, hwid)
	local db = loadKeyDB()
	if db[key] then db[key].hwid = hwid or ""; saveKeyDB(db); print("[LD] HWID set for " .. key .. ": " .. (hwid or "none")); return true end
	return false
end
...
print("=== Lil Durk 1.0 loaded ===")
print("> LD_GenKey('30d') | generate a key  (30d/1y/6h/15m/60s)")
print("> LD_ListKeys()   | list all keys with status")
print("> LD_FreezeKey(k) | freeze a key")
print("> LD_UnfreezeKey(k)| unfreeze a key")
print("> LD_SetHWID(k,u) | bind key to a Roblox user")
